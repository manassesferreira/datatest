...
then
    ...
    hive ${hive_conf_properties} -e "CREATE DATABASE IF NOT EXISTS DATATEST"
    hive ${hive_conf_properties} --hiveconf hive.lazysimple.extended_boolean_literal=true --hivevar hdfs_tmp_dir=${hdfs_tmp_dir} --database DATATEST -f ${KYLIN_HOME}/sample_cube/create_datatest_tables.sql  || { exit 1; }
fi
